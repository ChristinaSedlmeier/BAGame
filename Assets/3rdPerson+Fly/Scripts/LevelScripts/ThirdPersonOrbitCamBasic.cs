...
public class ThirdPersonOrbitCamBasic : MonoBehaviour 
{
	...
	// Offset to relocate the camera related to the player position.
	...
	void Awake()
	{
		...
		{
		//cam = transform;
		//desiredPosition = player.position + Quaternion.identity * pivotOffset + Quaternion.identity * camOffset;
		//// Set camera default position.
		//cam.position = Vector3.Lerp(transform.position, desiredPosition, 0.5f);
		...
		smoothPivotOffset = Vector3.Lerp(smoothPivotOffset, targetPivotOffset, smooth * Time.deltaTime);
				smoothCamOffset = Vector3.Lerp(smoothCamOffset, targetPivotOffset, smooth * Time.deltaTime);

		//cam.position = player.position + cam.rotation * smoothPivotOffset + smoothCamOffset;
		//cam.position = player.position + cam.rotation * smoothPivotOffset;
		//cam.position = Vector3.Lerp(transform.position, desiredPosition, 0.125f);
